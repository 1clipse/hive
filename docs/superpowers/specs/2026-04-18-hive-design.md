...
> **2026-09-05 外部主控模式修订（issue #57，用户已确认）**：本文件中主控必须运行于 Hive PTY、选择主控引擎、成员通过 stdin 向主控回灌等规定，继续适用于内部模式。新建工作区可选择 Codex App 外部主控模式：App 会话为唯一运行的主控，Hive 只管理成员进程；成员仍通过原 team 协议汇报，由 Runtime 持久化后通知绑定的 App 会话。外部主控按每次 MCP 调用的宿主元数据识别，经 Hive 面板确认绑定；不得通过手填 thread ID 或复制内部 Agent token 冒充身份。外部模式不自动启动内部主控，任务/汇报未清空不得换绑。接收结果不等于验收；通知不构成新的用户授权。具体接口和验证边界见 [Codex 外部主控实施计划](../../plans/2026-09-05-codex-external-controller.md)。

**日期**：2026-04-18
**状态**：Draft / Pending review
**作者**：brainstorming session

> **2026-09-05 协作修订**：任务内消息、关联派单、历史答疑、报告输入水位及提示词层次以 [面向工作的协作](2026-09-05-task-collaboration.md) 为准。该修订明确替代本文的禁止所有成员直连、缺信息即 report 结算等规则；名称派单、workspace 隔离和三态不变。验收与发布状态见修订实施记录。
...
> **2026-09-09 薄蜂巢修订（issue #75 / discussion #76）**：接入/待命就绪由 runtime 检测（记 `startup_ready_at`），不再要求 `team status` 握手；该命令仍可用于显式备注。工作目录默认共享 workspace 根；唯一例外是 workflow `agent({ isolation: 'worktree' })`（脚本 fan-out 时 orch 无法事先拆文件），`team spawn` / 常驻成员仍共享根。边界见 [设计决策](../../design-decisions.md)。
...
3. 用户点 [+ 添加 Workspace]，弹出选择器：
   - macOS / Linux 用 OS 系统目录选择器选 ~/projects/my-app
   - Windows 用浏览器内"浏览服务器文件系统"选择盘符/目录（原生 picker 可能被浏览器窗口遮挡）
   - 或手动粘贴绝对路径
...
team status "<current status>" [--artifact <path>]            # 可选：无 open dispatch 时写接入/待命/阻塞备注，不消耗 pending_task_count
...
接入/待命就绪由 runtime 检测（记 `startup_ready_at`），不再要求 worker 调 `team status`。无 open dispatch 时，worker 仍可用 `team status` 写显式备注（待命、阻塞等）；该命令记录 `status` 消息并尽力注入 orch stdin，但不递减 `pending_task_count`，也不关闭 dispatch。
...
点 [+ Add Workspace]：
1. 弹对话框，按平台提供添加方式：
   - **A. OS 系统目录选择器**（macOS / Linux 默认，点 "Browse..." 调出原生 picker）
   - **B. 浏览服务器文件系统**（Windows 默认；从虚拟 "This PC" 盘符列表进入，可上级返回盘符列表）
   - **C. 手动粘贴绝对路径**（输入框 + 验证）
...
| 数据库 | Node 内置 `node:sqlite`（Node 22.18+ / 24+） | 保留 SQLite 文件和 SQL 协议，首次安装无需下载或编译 bindings；同步嵌套事务由薄封装提供 |
| 文件监听 | chokidar | 跨平台稳定 |
...
- ❌ 全量 / 持久成员 Worktree 隔离（默认共享根目录，并发冲突由 orch 拆分负责）。**窄例外（2026-09）**：仅 workflow `agent({ isolation: 'worktree' })` 可把 ephemeral worker 放进临时 git worktree——脚本 fan-out 时 orch 无法事先拆文件；见 [设计决策](../../design-decisions.md)
- ❌ MCP 工具适配
...
- ⚠️ **任务图依赖建模** — 原 MVP 非目标（缩进 = 子任务分解，不是依赖关系）。**2026-06 经 user 同意放开此非目标**（spec 已过时）：新增一个**只读、可选**的安全子集——`.hive/tasks.md` 里 task 行可带尾随 `[needs: #2, #5]` 标注，`team next` 据此返回当前未被阻塞的任务（依赖全部 done）。仍**不做**自动派发、**不回写** tasks.md、**不改** web 端渲染（仅服务端 `task-deps.ts` 解析）。
...
| 风险 | 缓解 |
|---|---|
| 多 worker 同时改一个文件（默认共享根） | 仍依赖 orch 拆分。唯一例外：workflow `agent({ isolation: 'worktree' })` 把并行冲突交给脚本合并；见 [设计决策](../../design-decisions.md) |
...
4. **v0.4**：可选 worktree 隔离（高级开关；workflow `agent({ isolation: 'worktree' })` 窄例外已落地，见 [设计决策](../../design-decisions.md)）
...
| Q8 | 工作目录隔离 | 默认共享根目录。**2026-09 窄例外**：仅 workflow `agent({ isolation: 'worktree' })`（脚本 fan-out 无法事先拆文件）；持久成员仍共享根。见 [设计决策](../../design-decisions.md) |

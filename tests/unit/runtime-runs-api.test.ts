...
describe('runtime runs api (unit)', () => {
  test('GET /api/runtime/runs/:runId returns 404 when the live run is gone', async () => {
    const dataDir = mkdtempSync(join(tmpdir(), 'hive-run-api-missing-'))
    ...
    const response = await fetch(`${baseUrl}/api/runtime/runs/missing-run`, {
      headers: { cookie },
    })
  ...
  test('GET /api/runtime/runs/:runId does not convert runtime errors into 404', async () => {
    const dataDir = mkdtempSync(join(tmpdir(), 'hive-run-api-error-'))
    tempDirs.push(dataDir)
    const store = createRuntimeStore({
      agentManager: createAgentManager(),
      dataDir,
    ...
    const response = await fetch(`${baseUrl}/api/runtime/runs/error-run`, {
      headers: { cookie },
    })
